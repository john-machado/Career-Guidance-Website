<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/style.css">
    <link rel="stylesheet" href="css/responsive-style.css">
    <link rel="stylesheet" href="css/following.css">
    <!-- Fontawesome -->
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css" integrity="sha512-Fo3rlrZj/k7ujTnHg4CGR2D7kSs0v4LLanw2qksYuRlEzO+tcaEPQogQ0KaoGN26/zrn20ImR1DfuLWnOo7aBA==" crossorigin="anonymous" referrerpolicy="no-referrer" />
    <title>Create Account</title>
</head>
<body>
    <!-- Navbar -->
    <div id="navbar">

        <ul class="nav-list">
          <li class="nav-element"><a href="index.html" class="nav-link">Home</a></li>
          <li class="nav-element"><a href="#" class="nav-link">About</a></li>
          <li class="nav-element"><a href="#" class="nav-link">Social</a></li>
        </ul>
  
        <div class="login-button">
          <a href="login.html">Login</a>
        </div>
        
      </div>

      <div class="main-social">

        <div class="social-nav">
          <ul>
            <li><a href="following.php" >Following</a></li>
            <li><a href="groups.php" class="active-social">Groups</a></li>
          </ul>
        </div>

        <div class="social-list">

          <div class="social-list-element">
            <div class="social-element-left">
              <a href="groupinfo.php">
                <div class="name">Group Name</div>
                <div class="username">Admin : Username</div>
                <div class="status">Group description Lorem, ipsum dolor sit amet consectetur adipisicing elit. Veritatis delectus quisquam rerum amet soluta adipisci laudantium quasi, accusantium vero</div>
              </a>  
            </div>
            <div class="social-element-right">
              <button class="remove">Unfollow</button>
            </div>
          </div>

          <div class="social-list-element">
            <div class="social-element-left">
              <a href="groupinfo.php">
                <div class="name">Group Name</div>
                <div class="username">Admin : Username</div>
                <div class="status">Group description Lorem, ipsum dolor sit amet consectetur adipisicing elit. Veritatis delectus quisquam rerum amet soluta adipisci laudantium quasi, accusantium vero</div>
              </a>  
            </div>
            <div class="social-element-right">
              <button class="remove">Unfollow</button>
            </div>
          </div>

          <div class="social-list-element">
            <div class="social-element-left">
              <a href="groupinfo.php">
                <div class="name">Group Name</div>
                <div class="username">Admin : Username</div>
                <div class="status">Group description Lorem, ipsum dolor sit amet consectetur adipisicing elit. Veritatis delectus quisquam rerum amet soluta adipisci laudantium quasi, accusantium vero</div>
              </a>  
            </div>
            <div class="social-element-right">
              <button class="remove">Unfollow</button>
            </div>
          </div>

          <div class="social-list-element">
            <div class="social-element-left">
              <a href="groupinfo.php">
                <div class="name">Group Name</div>
                <div class="username">Admin : Username</div>
                <div class="status">Group description Lorem, ipsum dolor sit amet consectetur adipisicing elit. Veritatis delectus quisquam rerum amet soluta adipisci laudantium quasi, accusantium vero</div>
              </a>  
            </div>
            <div class="social-element-right">
              <button class="remove">Unfollow</button>
            </div>
          </div>

        </div>
      </div>
</body>
</html>